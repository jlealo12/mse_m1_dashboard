...
vsmTbl <- pData %>% 
  ...
  mutate(Count = n()) %>%
  group_by(categoria,sex,Count) %>%
  summarise_at(vars(time.min), funs(mean, median, sd, mad)) 

knitr::kable(vsmTbl,digits = 2, col.names = c("Categoría","Sexo","No. participantes","Tiempo promedio","Mediana tiempo carrera","SD tiempo","MAD tiempo"))
```
>Todos los indicadores de tiempo se presentan en minutos.
...
Análisis distribucional {data-orientation=columns}
================================================
...
-----------------------------------------------
Las distribuciones presentadas muestran que:

- Tanto los tiempos, como las velocidades de carrera, muestran distribuciones nromalizadas y con poco sesgo.
- Se observa un valor atípico, correspondiente a un participante Hombre de la categoría 5 (veteranos C).
- La baja representatividad de la categoría juvenil dificulta la visualización de los resultados.


Análisis de correlación {data-orientation=columns}
================================================
Column {.tabset .tabset-fade}
-----------------------------------------------
### Tiempos vs edad
...
> Para facilitar la visualización se eliminan valores atípicos del tiempo (>120 min).
...
Column {data-width=200}
-----------------------------------------------
Las visualizaciones detalladas permiten observar que:

- Existe una alta dispersión en los datos que dificulta la identificación de tendencias.
- El factor que mayor diferencia genera en los tiempos de carrera es el sexo del participante.
- Adicionalmente, parece existir una relación no lineal del tiempo con la edad del participante.Sin embargo, estas variaciones se generan por las categorías extremas (juveniles y veteranos C).